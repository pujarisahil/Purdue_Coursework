...
int llist_add(LINKED_LIST * list, char * name, char * value) {
	...
	e->name = strdup(name);
	e->value = strdup(value); // We dothis only in the linked-list. The value is always char *.
	...
	e = list->head->next; //adding to the entry
	while (e != list->head) {
		if (strcmp(e->name, name) == 0) { //check if name matches
			e->value = strdup(value);
			return 1;
		}
		else {
			e = e->next; //move to the next one otherwise
		}
	}

	// If entry is not found, add new entry at the end of the list
	...
	list->nElements++; //increment the number of elements in the list
	return 0;
}
...
char * llist_lookup(LINKED_LIST * list, char * name) {
	...
	e = list->head->next;
	while (e != list->head) {
		if (strcmp(e->name, name) == 0) { //check if the name is same 
			return (e->value); //return the value of that particular name if the name is same
		}
		else {
			e = e->next; //otherwise move to the next one
		}
	}
